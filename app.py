...
st.set_page_config(page_title="Requirements Analyzer", layout="wide")
...
def load_saved_state():
    ...
    try:
        with open(STATE_FILE, "r", encoding="utf-8") as f: saved_state = json.load(f)
        st.session_state.requirements_text = saved_state.get("requirements_text", "")
        st.session_state.analysis_result = saved_state.get("analysis_result")
        st.session_state.log_file = saved_state.get("log_file")
...
with col1:
    if st.button("Load Sample Requirements"): st.session_state.requirements_text = SAMPLE_REQUIREMENTS
with col2:
    if st.button("Clear Saved State"):
        clear_saved_state()
        st.rerun()
threshold = st.slider("Satisfaction Threshold", min_value=0.0, max_value=1.0, value=0.80, step=0.05)
requirements_text = st.text_area("Enter Requirements (one per line)", key="requirements_text", height=300)
...
if st.session_state.analysis_result:
    ...
    st.metric("Satisfaction Score", f"{result['satisfaction']:.2f}")
    if result["satisfaction"] >= threshold:
        st.success(f"Requirements meet the threshold ({threshold:.2f}).")
    else:
        st.warning(f"Requirements are below the threshold ({threshold:.2f}).")
    for category, issues in result["issues"].items():
...
if (st.session_state.log_file and os.path.exists(st.session_state.log_file)):
    ...
    with open(st.session_state.log_file, "r", encoding="utf-8") as f: report_content = f.read()
